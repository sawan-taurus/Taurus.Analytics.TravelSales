...
# Source_Type = "Oasis"  # or "Atoz" or "Atoz"
...
if Data_Load_Type == "historical":
    ...
    silver_df_filtered = silver_df.withColumn("QuoteDateOnly", to_date(col("QuoteDate"))).filter(
        (col("QuoteDateOnly") >= to_date(lit("2026-01-01"))) &
...
dim_agent_path = f"abfss://{WORKSPACE}@onelake.dfs.fabric.microsoft.com/{Gold_LAKEHOUSE}.Lakehouse/Tables/DimAgent"   # ADD THIS
...
silver_df_filtered = silver_df_filtered.withColumnRenamed("AgentID", "SourceAgentID")   # ADD THIS
...
# Join with DimAgent to get AgentId
silver_enriched_df = silver_enriched_df.withColumn("AgentName_Clean", upper(trim(col("AgentName"))))
...
silver_enriched_df = silver_enriched_df.join(
    ...
    on="AgentName_Clean",
    how="left"
).drop("AgentName_Clean")

silver_enriched_df = silver_enriched_df.withColumn(
    "AgentId", when(col("AgentId").isNull(), lit(0)).otherwise(col("AgentId"))
)
...
fact_df = silver_enriched_df.groupBy(
    ...
    "ChannelId", "OptionId", "DestinationId", "AgeGroupId", "MedicalScoreId", "FamilyGroupId","MarketingChannelId", "AgentId" 
).agg(
