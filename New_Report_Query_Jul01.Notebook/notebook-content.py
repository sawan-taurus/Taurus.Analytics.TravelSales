...
SELECT Date, 'SOI', 'SOI AGG' AS SchemeSubset, 
    ...
    EstMonEndRetailAgg AS EstMonEndRetail, 
    ...
    ROUND(
        CASE 
            ...
            THEN ROUND(EstMonEndVolAgg  / BudgetVol * 100,0)
            ELSE NULL 
        END, 2) AS PercentToVol,
        ROUND(
    CASE 
        WHEN BudgetGWP IS NOT NULL AND BudgetGWP > 0 
        THEN ROUND(EstMonEndRetailAgg / BudgetGWP * 100 ,0)
        ELSE NULL 
...
SELECT Date, 'SOI', 'SOI Direct' AS SchemeSubset, 
    ...
    EstMonEndRetailDirect AS EstMonEndRetail, 
