...
TodaysSubSetsTotal AS (
    SELECT 
        ...
        --ROUND(SUM(EstMonEndRetailAgg) / CAST(day(dd.Date) AS INT) * day(last_day(dd.Date)) ,2) AS EstMonEndRetailAgg,
...
(SELECT SUM(EstMonEndNetMargin) FROM NetMarginValue) AS EstMonEndNetMargin,
...
ROUND(((SELECT SUM(EstMonEndNetMargin) FROM NetMarginValue) * 100 / (SELECT SUM(BudgetNetMargin) FROM EverythingInOne WHERE SchemeSubset IN ('Trusted Total','SOI Total','Viva Total','Oasis Total','Atoz Total'))), 2) AS PercentToNetMargin
