...
TotalOasisRow AS (
    SELECT 
        ...
        ROUND((SUM(fdo.TotalGrossIncIPTMonth) * 0.89) / SUM(fdo.VolumeMonth),2)  * 
...
TotalAtozRow AS (
    SELECT 
        ...
        ROUND(SUM(fdo.VolumeMonth) / CAST(day(dd.Date) AS INT), 0)  AS AvgPolPerDay,
        ROUND((SUM(fdo.TotalGrossIncIPTMonth)) / SUM(fdo.VolumeMonth),2) AS AvgP,
        ROUND(SUM(fdo.VolumeMonth) / CAST(day(dd.Date) AS INT) * day(last_day(dd.Date))) AS EstMonEndVol,
        --ROUND((SUM(fdo.TotalGrossIncIPTMonth)) / SUM(fdo.VolumeMonth) * SUM(fdo.VolumeMonth) / CAST(day(dd.Date - 1) AS INT) * 
        --    day(last_day(dd.Date)),2) AS EstMonEndRetail ,

        ROUND((SUM(fdo.TotalGrossIncIPTMonth)) / SUM(fdo.VolumeMonth),2)  * 
        ROUND(SUM(fdo.VolumeMonth) / CAST(day(dd.Date) AS INT) * day(last_day(dd.Date))) AS EstMonEndRetail,
        ...
        ROUND((SUM(fdo.TotalGrossIncIPTMonth)) / SUM(fdo.VolumeMonth) * SUM(fdo.VolumeMonth) / CAST(day(dd.Date - 1) AS INT) * 
            day(last_day(dd.Date)) / 1.2 * ((TaurusRetained / GPW) * 0.9) , 2) AS EstMonEndNetMargin,
        CONCAT(CAST(ROUND(((SUM(fdo.TotalGrossIncIPTMonth)) / SUM(fdo.VolumeMonth) * SUM(fdo.VolumeMonth) / DAY(dd.Date - 1) * 
            DAY(LAST_DAY(dd.Date)) / 1.2 * ((TaurusRetained / GPW) * 0.9) / TaurusRetained) * 100, 0) AS INT), '%') AS PerToFCastRetail,
        CONCAT(CAST(ROUND(((SUM(fdo.TotalGrossIncIPTMonth)) / SUM(fdo.VolumeMonth) * SUM(fdo.VolumeMonth) / DAY(dd.Date - 1) * 
            DAY(LAST_DAY(dd.Date)) / 1.2 * ((TaurusRetained / GPW)) / TaurusRetained) * 100, 2) AS DECIMAL(10,2)), '%') AS PerToFCastMargin,
        ROUND(GPW * 1.2,1) as PerToFCastNo,
        GPW ,TaurusRetained,TaurusRetained,
        CONCAT(ROUND((TaurusRetained / GPW) * 0.9 * 100, 2),'%') AS AvGrossMargin,
        CONCAT(ROUND((TaurusRetained / GPW) * 0.9 * 100, 2),'%')  AS AvNetMargin,
...
TotalVivaRow AS (
    SELECT 
        ...
        ROUND(SUM(fds.TotalGrossIncIPTMonth) / SUM(fds.VolumeMonth), 2)  * 
        ROUND(SUM(fds.VolumeMonth) / CAST(day(dd.Date) AS INT) * day(last_day(dd.Date)))  AS EstMonEndRetail,
...
TodaysSubSetsTotal AS (
    SELECT 
        ...
        (ROUND(SUM(MTDRetail) / SUM(MTDVol),2)) * 
        (ROUND(SUM(MTDVol) / CAST(day(Date) AS INT) * day(last_day(Date))) ) AS EstMonEndRetail,
...
EverythingInOne AS (
---------------------------------TotalSubSet--------------------------------------------------
    --SELECT Date, 'TotalSubSet' as TotalSubSet, 'TotalSubSet' AS SchemeSubset,DateVol,DateRetail,MTDVol,MTDRetail,AvgPolPerDay,AvgP,EstMonEndVol,EstMonEndRetail,
    --EstMonEndNetMargin,NULL as BudgetVol,NULL as BudgetGWP,NULL as BudgetNetMargin,NULL as PercentToVol,NULL as PercentToGWP,NULL as PercentToNetMargin
    --FROM TodaysSubSetsTotal 
...
SELECT Date, 'Trusted', 'Trusted Total' AS SchemeSubset, 
    ...
    (COALESCE(MTDVolAgg, 0) + COALESCE(MTDVolDirect, 0)) / 2 AS AvgPolPerDay,
...
SELECT Date, 'Atoz', 'Atoz AGG' AS SchemeSubset, 
    ...
    ROUND(CASE WHEN BudgetVol IS NOT NULL AND BudgetVol > 0 THEN (EstMonEndVolAgg / BudgetVol) * 100 ELSE NULL END, 2) AS PercentToVol,
    ROUND(CASE WHEN BudgetGWP IS NOT NULL AND BudgetGWP > 0 THEN (EstMonEndRetailAgg / BudgetGWP) * 100 ELSE NULL END, 2) AS PercentToGWP,
    ROUND(
    CASE 
        WHEN BudgetNetMargin IS NOT NULL AND BudgetNetMargin > 0 
        THEN (EstMonEndNetMarginAgg *(
...
SELECT Date, 'Atoz', 'Atoz Direct' AS SchemeSubset, 
    ...
    ROUND(CASE WHEN BudgetVol IS NOT NULL AND BudgetVol > 0 THEN (EstMonEndVolDirect / BudgetVol) * 100 ELSE NULL END, 2) AS PercentToVol,
    ROUND(CASE WHEN BudgetGWP IS NOT NULL AND BudgetGWP > 0 THEN (EstMonEndRetailDirect / BudgetGWP) * 100 ELSE NULL END, 2) AS PercentToGWP,
...
SELECT t.Date,  'TotalSubSet' AS TotalSubSet,  'TotalSubSet' AS SchemeSubset, t.DateVol, t.DateRetail, t.MTDVol, t.MTDRetail, t.AvgPolPerDay,
 t.AvgP, t.EstMonEndVol, t.EstMonEndRetail, (SELECT SUM(EstMonEndNetMargin) FROM NetMarginValue) AS EstMonEndNetMargin, NULL AS BudgetVol,
 NULL AS BudgetGWP, NULL AS BudgetNetMargin, NULL AS PercentToVol, NULL AS PercentToGWP, NULL AS PercentToNetMargin
FROM TodaysSubSetsTotal t
WHERE t.Date = '${report_date}'
union all 
SELECT Date, TotalSubSet, SchemeSubset,DateVol,DateRetail,MTDVol,MTDRetail, AvgPolPerDay, AvgP, EstMonEndVol, EstMonEndRetail, EstMonEndNetMargin,
