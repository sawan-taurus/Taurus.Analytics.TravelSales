...
TotalOasisRow AS (
    SELECT 
        ...
        ROUND(SUM(fdo.VolumeMonth) / CAST(day(dd.Date - 1) AS INT), 0)  AS AvgPolPerDay,
        ROUND((SUM(fdo.TotalGrossIncIPTMonth) * 0.89) / SUM(fdo.VolumeMonth),2) AS AvgP,
        ROUND(SUM(fdo.VolumeMonth) / CAST(day(dd.Date - 1) AS INT) * day(last_day(dd.Date))) AS EstMonEndVol,
...
AggregatedData AS (
    SELECT 
        ...
        ROUND(SUM(fds.VolumeMonth) / CAST(day(dd.Date - 1) AS INT), 0)  AS AvgPolPerDay,
        ROUND(SUM(fds.TotalGrossIncIPTMonth) / SUM(fds.VolumeMonth), 2) AS AvgP,
        ROUND(SUM(fds.VolumeMonth) / CAST(day(dd.Date - 1) AS INT) * day(last_day(dd.Date))) AS EstMonEndVol,
...
SOITotalRow AS (
    SELECT 
        ...
        0 as PerToFCastRetail,
        0 as PerToFCastMargin,
        ROUND(GPW * 1.2,1) as PerToFCastNo,
        GPW,
...
SELECT 
    ...
    ROUND(st.EstMonEndNetMargin + COALESCE(aggs.EstMonEndNetMargin, 0) + COALESCE(direct.EstMonEndNetMargin, 0), 2) as EstMonEndNetMargin, CONCAT(CAST(ROUND((st.EstMonEndRetail / st.PerToFCastNo), 0) AS INT), '%') as PerToFCastRetail,
    CONCAT(CAST(ROUND((st.EstMonEndGrossMargin + COALESCE(aggs.EstMonEndGrossMargin, 0) + COALESCE(direct.EstMonEndGrossMargin, 0)) / (st.TaurusGrossRetained + COALESCE(aggs.TaurusGrossRetained, 0) + COALESCE(direct.TaurusGrossRetained, 0)), 0) AS INT), '%') as PerToFCastMargin,
...
# -- %%sql 
# -- -- CREATE TABLE TaurusGoldLH.DimSchemeSubSet (
# -- --     SchemeSubsetId BIGINT NOT NULL,
# -- --     SchemeSubset STRING NOT NULL
# -- -- )
# -- -- USING DELTA;
...
# MAGIC %%sql 
# MAGIC drop table TaurusGoldLH.FactDailyReport
...
# MAGIC %%sql 
# MAGIC CREATE TABLE TaurusGoldLH.FactDailyReport (
# MAGIC     DateId BIGINT,
...
# MAGIC %%sql 
# MAGIC select distinct DateId  from FactDailyReport
# MAGIC order by 1 
# MAGIC 
...
# MAGIC %%sql 
# MAGIC 
# MAGIC select sum(DateVol) from FactDailyReport f
# MAGIC join DimSchemeSubSet s on f.SchemeSubsetId = s.SchemeSubsetId
# MAGIC where DateId in (20250401,20250402,20250403,20250404,20250405,20250406,20250407,20250408,20250409,20250410,20250411,20250412,20250413,20250414,20250415) 
# MAGIC and SchemeSubset in ('SOI','Trusted','Viva','Atoz','Oasis')
...
# MAGIC %%sql 
# MAGIC 
# MAGIC select * from FactDailyReport
